      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <nav className="hidden md:flex space-x-8">
            {navigation.map(item => <Link key={item.name} to={item.href} className="text-zeli-gray hover:text-zeli-yellow font-medium transition-colors duration-200">
                {item.name}
              </Link>)}
          </nav>
        ...
        {isMenuOpen && <div className="md:hidden pb-4">
            <div className="flex flex-col space-y-3">
              {navigation.map(item => <Link key={item.name} to={item.href} className="text-zeli-gray hover:text-zeli-yellow font-medium py-2" onClick={() => setIsMenuOpen(false)}>
                  {item.name}
                </Link>)}
